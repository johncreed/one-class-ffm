...
void ImpProblem::save_npy_files() {
    const ImpLong shape_p[] = {Uva->m,  k};
    const ImpLong shape_q[] = {Uva->n,  k};
    for(ImpInt fi = 0; fi < f ; fi++){
        for(ImpInt fj = fi; fj < f; fj++){
            ImpInt fij = index_vec(fi, fj, f);
            if( !param->self_side && !(fi < fu && fj >= fu) )
                continue;
            npy::SaveArrayAsNumpy("./Pva"+to_string(fij), false, 2, shape_p, Pva[fij]);
            npy::SaveArrayAsNumpy("./Qva"+to_string(fij), false, 2, shape_q, Qva[fij]);
        }
    }
}
